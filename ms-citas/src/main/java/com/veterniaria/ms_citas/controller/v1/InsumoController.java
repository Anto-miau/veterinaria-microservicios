...
public class InsumoController {
    ...
    @GetMapping
    public ResponseEntity<List<InsumoDTO>> todas() {
        List<InsumoDTO> lista = insumoService.obtenerTodos();
    ...
    @PostMapping
    public ResponseEntity<?> registrar(@Valid @RequestBody Insumo insumo) {
        try {
            InsumoDTO dto = insumoService.guardar(insumo);
    ...
    public ResponseEntity<?> actualizar(@PathVariable Integer id, @RequestBody Insumo insumo) {
        try {
            InsumoDTO dto = insumoService.actualizar(id, insumo);
            return new ResponseEntity<>(dto, HttpStatus.OK);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> eliminar(@PathVariable Integer id) {
        String resultado = insumoService.eliminar(id);
