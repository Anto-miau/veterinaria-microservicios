...
public class CitaController {
    ...
    @GetMapping
    public ResponseEntity<List<CitaDTO>> todas() {
        List<CitaDTO> lista = citaService.obtenerTodos();
    ...
    @PostMapping
    public ResponseEntity<?> registrar(@Valid @RequestBody Cita cita) {
        try {
            CitaDTO dto = citaService.guardar(cita);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> actualizar(@PathVariable Integer id, @RequestBody Cita cita) {
        try {
            CitaDTO dto = citaService.actualizar(id, cita);
            return new ResponseEntity<>(dto, HttpStatus.OK);
        } catch (RuntimeException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
        }
